...
def init_problem(cnf_problem: CNF, batch_size: int, key: jnp.ndarray = None, learning_rate: float = 1.0):
    ...
    return embedding, optimizer, var_tensor, sign_tensor, labels
...
# (assignment_func(params)>0.5).astype(int)
    ...
    return params, epoch+1, loss_value, end_t - start_t, solutions
...
def run_back_prop_verbose(
    ...
    def get_solutions(
        ...
        assignment = (params>0.5).astype(int)
        # assignment = jnp.array(jax.nn.sigmoid(params) > 0.5, dtype=jnp.int32)
        sat = jnp.take(assignment, var_tensor, fill_value=-1, axis=1)
        sat = jnp.where(sign_tensor > 0, 1 - sat, sat)
        sat = jnp.all(jnp.any(sat>0, axis=2), axis=1)
        return jnp.take(assignment, jnp.where(sat)[0], axis=0)
    ...
    def compute_loss(
        params: jnp.ndarray, var_tensor: jnp.ndarray, sign_tensor: jnp.ndarray, labels: jnp.ndarray, loss
    ):  
        # x = jax.nn.sigmoid(params)
        x = jnp.take(params, var_tensor, fill_value=0.0, axis=1)
        x = jnp.where(sign_tensor > 0, x, 1 - x)
        x = 1 - jnp.prod(x, axis=-1)
        return loss(x, labels).mean()
        # return optax.sigmoid_binary_cross_entropy(x, labels).mean()
    ...
    def step(
        ...
        # l = compute_loss(params, var_tensor, sign_tensor, labels, loss)
        loss_value, grads = jax.value_and_grad(compute_loss)(
            params, var_tensor, sign_tensor, labels, loss
        )
        updates, opt_state = optimizer.update(grads, opt_state)
        params = optax.apply_updates(params, updates)
        return params, opt_state, loss_value

    loss = optax.sigmoid_binary_cross_entropy if loss_fn_str == "sigmoid_binary_cross_entropy" else optax.l2_loss
    params = jax.nn.sigmoid(params)
    og = params
    opt_state = optimizer.init(params)
    ...
    return params, epoch+1, loss_value, 0.0, solutions
...
if __name__ == "__main__":
    ...
    circuit = SATCircuit(
        use_pgates=True, key=jax.random.PRNGKey(0), batch_size=1000,
    )
    ...
    final_params, epoch, loss_value, elapsed_time = circuit.fit(
        num_epochs=1000,
        params=params,
        optimizer=optimizer,
        var_tensor=var_tensor,
    ...
    print(f"step {epoch}, loss: {loss_value}")
