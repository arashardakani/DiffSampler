...
class Circuit(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(len(self.problems.clauses)):
            ...
            y = torch.where(torch.FloatTensor(self.problems.clauses[i]).to(x.device) > 0., x[:,idx], 1. - x[:,idx])
...
def main():
    ...
    for epoch in range(args.num_train_epochs):
        ...
        print('Probability of getting 1 as the output @ epoch', epoch,':', (1. - torch.round(l * 16.)/16.).item())
        l.backward()
        optim.step()
    print('\nSolution:', ((torch.sign(model.EMB.data.weight.data) + 1.0)/2.).long().cpu().tolist(),'\n' )
