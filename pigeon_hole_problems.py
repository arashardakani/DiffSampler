...
class Circuit(nn.Module):
    def __init__(self, problems):
        ...
        self.layer = {}
        self.layer = nn.ModuleList([OR(len(problems.clauses[i]))for i in range(len(problems.clauses))])
        for i in range(len(problems.clauses)):
            for j in range(len(problems.clauses[i])):
                self.layer[i].dense.weight.data[0,j].mul_(torch.sign( torch.tensor(problems.clauses[i][j]) ))
    ...
    def forward(self, input):
        intermediate_out = torch.zeros(input.size()[0],len(self.problems.clauses)).to(input.device)
        x = self.EMB(input)
...
class EMB(nn.Module):
    def __init__(self, num_in):
        ...
        self.data.weight.data[0,0] = 1.
        self.data.weight.data[0,1] = -1.
        self.data.weight.data[0,2] = -1.
        self.data.weight.data[0,3] = 1.
